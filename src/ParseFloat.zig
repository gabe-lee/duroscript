...
const U128 = struct {
    ...
    fn new(lo_bits: u64, hi_bits: u64) U128 {
        return U128{
            .lo_bits = lo_bits,
            .hi_bits = hi_bits,
        };
...
pub fn parse_float_from_decimal_parts(mantissa: u64, exponent: i64, negative: bool, slow_buf: SlowParseBuffer, slow_idx: usize) f64 {
    assert(mantissa <= F64.MAX_SIG_DECIMAL);
    ...
    // slow path
    return handle_slow_parse(slow_buf[0..slow_idx], negative);
}
...
fn try_eisel_lemire_path(mantissa10: u64, exponent10: i64, negative: bool) ?f64 {
    ...
    const value_ex = compute_product_approx: {
        ...
        var first = U128.mul(mantissa2, pow5.lo_bits);

        if (first.hi_bits & mask == mask) {
            const second = U128.mul(mantissa2, pow5.hi_bits);
        ...
        break :compute_product_approx U128.new(first.lo_bits, first.hi_bits);
    };
...
fn handle_slow_parse(slow_parse_slice: []const u8, negative: bool) f64 {
    ...
    var d = Decimal.parse(slow_parse_slice);

    //VERIFY these checks should have been handeled before here
    if (d.num_digits == 0 or d.decimal_point < Decimal.min_exponent) {
        std.debug.panic("slow float parse resulted in zero, input = {s}", .{slow_parse_slice});
    } else if (d.decimal_point >= Decimal.max_exponent) {
...
const EISEL_LEMIRE_POWERS_OF_FIVE_128 = [_]U128{
